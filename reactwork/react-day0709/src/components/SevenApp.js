...
const SevenApp = () => {
    ...
    return(
        <div>
            ...
            {
                [...array1].map((item,idx) =>
                    <img src={require(`../food/${idx+1}.jpg`)} className={'photo'}/>
                )
            }
            <hr/>
            <h5>image 의 1.jpg ~ 20.jpg 까지 이미지 출력</h5>
            {
                [...new Array(20)].map((item,idx) =>
                    <img src={require(`../image/${idx+1}.jpg`)} className={'photo'}/>
                )
            }
            <hr/>
            <h5>foodArray 배열안의 객체값 출력</h5>
            {
                ...
                <figure key={idx} style={{float:'left', marginRight:'10px'}}>
                    <img src={require(`../food/${item.photo}`)} className={'photo'} style={{marginBottom:'10px'}}/>
                    <figcaption>
